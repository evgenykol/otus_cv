...
## Комментарии Антона с лекции по порядку выполнения

1. Выбрать задачу, решаемую сетью. Примем, что будем делать классификацию
2. Выбрать сеть, выбрать датасет и обучить сеть без аугментаций
3. Замерять точность
4. Применить несколько стратегий аугментаций и оценить точность классификации для каждой из стратегий:
   - базовые аугментации
   - средние аугментации
   - мощные аугментации
5. Для аугментаций можно использовать следующие библиотеки:
   - torchvision
   - albumentation
   - OpenCV
6. Обобщить результаты в сводную таблицу